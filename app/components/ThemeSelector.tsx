...
const IconBase: React.FC<React.SVGProps<SVGSVGElement>> = ({
  children,
  ...props
}) => {
  return (
...
const findTheme = (theme?: string): Theme => {
  // TODO: get local storage item
  // TODO: if not found, use system theme
  // TODO: if not found, use light theme
...
const ThemeSelector: React.FC = (props) => {
  ...
  useEffect(() => {
    if (selectedTheme) {
      ...
      const dataTheme = themes.find(
        (theme) =>
          theme.value ===
          (document.documentElement.getAttribute("data-theme") as string)
      );
      if (dataTheme) setSelectedTheme(dataTheme);
    }
  }, [selectedTheme]);
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <Listbox.Button className="shadow-black/5 ring-black/5 dark:ring-white/5 flex h-6 w-6 items-center justify-center rounded-lg shadow-md ring-1 dark:bg-slate-700 dark:ring-inset">
        ...
        <Listbox.Options className="shadow-black/5 ring-black/5 dark:ring-white/5 absolute top-full left-1/2 z-50 mt-3 w-36 -translate-x-1/2 space-y-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md ring-1 dark:bg-slate-800">
          {themes.map((theme) => (
            <Listbox.Option
              key={theme.value}
              value={theme}
                  ...
                  <div className="dark:ring-white/5 rounded-md bg-white p-1 shadow ring-1 ring-slate-900/5 dark:bg-slate-700 dark:ring-inset">
                    <theme.icon
                      className={clsx("h-4 w-4", {
                        "fill-primary dark:fill-primary": selected,
